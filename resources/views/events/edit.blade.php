    ...
    <form action="{{ route('events.update', $event->id) }}" method="POST" class="space-y-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div class="col-span-2">
                ...
                <input type="text" name="event_name" value="{{ old('event_name', $event->event_name) }}" class="w-full mt-1 p-2 border rounded-md" required>
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700">विभाग</label>
                <input type="text" name="department" value="{{ old('department', $event->department) }}" class="w-full mt-1 p-2 border rounded-md">
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700">सुरु हुने मिति</label>
                <input type="date" name="start_date" value="{{ old('start_date', $event->start_date) }}" class="w-full mt-1 p-2 border rounded-md">
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700">सकिने मिति</label>
                <input type="date" name="end_date" value="{{ old('end_date', $event->end_date) }}" class="w-full mt-1 p-2 border rounded-md">
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700">सुरु हुने समय</label>
                <input type="time" name="start_time" value="{{ old('start_time', $event->start_time) }}" class="w-full mt-1 p-2 border rounded-md">
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700">सकिने समय</label>
                <input type="time" name="end_time" value="{{ old('end_time', $event->end_time) }}" class="w-full mt-1 p-2 border rounded-md">
            </div>
        </div>
        ...
        <div class="flex items-center gap-2 bg-gray-50 p-3 rounded-md">
            <input type="checkbox" name="is_tiffin_eligible" value="1" {{ old('is_tiffin_eligible', $event->is_tiffin_eligible) ? 'checked' : '' }} class="w-5 h-5 text-blue-600">
