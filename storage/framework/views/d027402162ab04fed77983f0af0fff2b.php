...
<div class="p-6">
    ...
    <a href="<?php echo e(route('repair.expenses.index', array_merge(request()->all(), ['export' => 'excel']))); ?>" class="bg-green-600 text-white px-3 py-2 rounded text-sm hover:bg-green-700 inline-block mb-3">
        Excel डाउनलोड
    </a>
        ...
        <table class="w-full border-collapse">
            ...
            <tbody>
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="p-3 border"><?php echo e($row['employee']->employee_code ?? '-'); ?></td>
                    <td class="p-3 border"><?php echo e($row['employee']->name ?? 'N/A'); ?></td>
                    <td class="p-3 border"><?php echo e($row['employee']->position->name ?? 'N/A'); ?></td>
                    <td class="p-3 border"><?php echo e($row['fy_year']); ?></td>
                    <td class="p-3 border"><?php echo e($row['bs_month']); ?></td>
                    <td class="p-3 border"><?php echo e($row['bs_date']); ?></td>
                    <td class="p-3 border"><?php echo e($row['description']); ?></td>
                    <td class="p-3 border text-right">रु <?php echo e(number_format($row['amount'], 2)); ?></td>
                    ...
                    <td class="p-3 border">
                        ...
                        <?php if(auth()->user()->role === 'admin' || \App\Models\RolePermission::where('role', auth()->user()->role)->where('permission', 'repair.expenses.manage')->exists()): ?>
                            <form action="<?php echo e(route('repair.expenses.toggleEdit', $row['expense_id'])); ?>" method="POST" class="inline">
                                <?php echo csrf_field(); ?>
                                <button type="submit" class="<?php echo e($row['isEdit'] ? 'bg-orange-500 hover:bg-orange-600' : 'bg-green-600 hover:bg-green-700'); ?> text-white px-3 py-1 rounded text-sm ml-1">
                                    <?php echo e($row['isEdit'] ? 'बन्द' : 'खुला'); ?>
