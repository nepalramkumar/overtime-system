        ...
        <div>
            ...
            <select name="event_id" class="w-full border border-gray-300 rounded-md px-2 py-2 text-sm">
                ...
                @foreach(\App\Models\Event::all() as $ev)
                    <option value="{{ $ev->id }}" {{ request('event_id') == $ev->id ? 'selected' : '' }}>{{ $ev->event_name }}</option>
                @endforeach
            </select>
        </div>
        <div class="flex gap-2 items-end">
            <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white text-xs px-3 py-2 rounded">🔍 खोज</button>
            <a href="{{ route('reports.finance') }}" class="bg-gray-500 hover:bg-gray-600 text-white text-xs px-3 py-2 rounded">Reset</a>
        </div>
    </div>
</form>
    ...
    <table class="w-full border-collapse">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td class="p-3 border">{{ $data->employee->designation ?? 'N/A' }}</td>
                ...
                {{-- Rate (employee बाट लिने) --}}
                <td class="p-3 border text-center">{{ number_format($data->employee->ot_rate ?? 0, 2) }}</td>
                {{-- OT Amount = Hours * Rate --}}
                <td class="p-3 border text-right font-bold">
                    रु {{ number_format($data->total_hours * ($data->employee->ot_rate ?? 0), 2) }}
                </td>
...
<div class="mt-4">
    <a href="{{ route('reports.exportFinanceExcel', request()->all()) }}" class="bg-green-600 text-white px-4 py-2 rounded shadow hover:bg-green-700 transition">
