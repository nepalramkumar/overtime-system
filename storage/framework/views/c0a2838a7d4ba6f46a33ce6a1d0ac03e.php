...
<div class="max-w-7xl mx-auto space-y-6">
    ...
    <!-- Search & Filter Form -->
    <form action="<?php echo e(route('overtime.verified')); ?>" method="GET" class="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-4 items-end">
            <div>
                <label class="block text-xs font-semibold text-slate-600 mb-1">From Date</label>
                <input type="date" name="from_date" value="<?php echo e(request('from_date')); ?>" class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none">
            </div>
            <div>
                <label class="block text-xs font-semibold text-slate-600 mb-1">To Date</label>
                <input type="date" name="to_date" value="<?php echo e(request('to_date')); ?>" class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none">
            </div>
            <div>
            ...
            <div class="flex items-center gap-2">
                ...
                <a href="<?php echo e(route('overtime.verified')); ?>" class="w-full bg-slate-100 hover:bg-slate-200 text-slate-700 font-medium text-xs px-4 py-2.5 rounded-lg border border-slate-300 transition text-center">
                    Reset
                </a>
            </div>
        </div>
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <?php $__empty_1 = true; $__currentLoopData = $records; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $rec): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <tr class="hover:bg-slate-50/80 transition">
                            ...
                            <td class="p-3.5 font-medium text-slate-800 whitespace-nowrap"><?php echo e($rec->ot_date); ?></td>
                            ...
                            <td class="p-3.5 text-slate-800"><?php echo e($rec->event->event_name ?? ($rec->remarks ?: 'सामान्य')); ?></td>
                            <td class="p-3.5 text-xs text-slate-600 font-medium">
                                <span class="inline-flex items-center gap-1 bg-slate-100 text-slate-700 px-2 py-1 rounded">
                                    ...
                                    <?php echo e($rec->verifier->name ?? 'N/A'); ?>
                            ...
                            <td class="p-3.5 text-center whitespace-nowrap">
                                <div class="flex items-center justify-center gap-1.5">
                                    ...
                                    <!-- Unverify Form -->
                                    <form action="<?php echo e(route('overtime.unverify', $rec->id)); ?>" method="POST" class="inline" onsubmit="return confirm('के तपाईं यो रेकर्ड Unverify गर्न चाहनुहुन्छ? यो फेरि Pending मा जान्छ।')">
                                        <?php echo csrf_field(); ?>
                                        <button type="submit" 
