        ...
        <table class="w-full border-collapse">
            ...
            <tbody>
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="p-3 border">{{ $expense->employee->name ?? 'N/A' }}</td>
                    <td class="p-3 border">{{ $expense->employee->position->name ?? 'N/A' }}</td>
                    <td class="p-3 border">{{ $expense->fy_year }}</td>
                    <td class="p-3 border text-right">रु {{ number_format($expense->total_amount, 2) }}</td>
                    ...
                    <td class="p-3 border">
                        <a href="{{ route('repair.expenses.edit', $expense->id) }}"
                        ...
                        @if(auth()->user()->role === 'admin' || \App\Models\RolePermission::where('role', auth()->user()->role)->where('permission', 'repair.expenses.manage')->exists())
                            <form action="{{ route('repair.expenses.toggleEdit', $expense->id) }}" method="POST" class="inline">
                                @csrf
                                <button type="submit" class="{{ $expense->isEdit ? 'bg-orange-500 hover:bg-orange-600' : 'bg-green-600 hover:bg-green-700' }} text-white px-3 py-1 rounded text-sm ml-1">
                                    {{ $expense->isEdit ? 'Edit बन्द गर्नुहोस्' : 'Edit खोल्नुहोस्' }}
                                </button>
                            </form>
                            <form action="{{ route('repair.expenses.destroy', $expense->id) }}" method="POST" class="inline" onsubmit="return confirm('के तपाईं पक्का डिलिट गर्न चाहनुहुन्छ?')">
                                @csrf
                                @method('DELETE')
