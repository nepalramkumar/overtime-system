...
<div class="p-6">
    <div class="flex justify-between items-center mb-4">
        ...
        <a href="<?php echo e(route('petrol.bills.create')); ?>" class="bg-blue-600 text-white px-4 py-2 rounded font-bold hover:bg-blue-700">
            + नयाँ Bill थप्नुहोस्
        </a>
    </div>
    ...
    <form action="<?php echo e(route('petrol.bills.index')); ?>" method="GET" class="bg-white border border-gray-200 rounded-lg p-3 mb-4 shadow-sm">
            ...
            <div class="w-64">
                ...
                <select name="petrol_month_id" class="w-full border border-gray-300 rounded-md px-2 py-2 text-sm">
                    ...
                    <?php $__currentLoopData = $months; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $m): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <option value="<?php echo e($m->id); ?>" <?php echo e(request('petrol_month_id') == $m->id ? 'selected' : ''); ?>><?php echo e($m->month); ?> - <?php echo e($m->year); ?></option>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                </select>
            </div>
            <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white text-xs px-3 py-2 rounded">🔍 खोज</button>
            <a href="<?php echo e(route('petrol.bills.index')); ?>" class="bg-gray-500 hover:bg-gray-600 text-white text-xs px-3 py-2 rounded">Reset</a>
        </div>
    </form>
        ...
        <table class="w-full border-collapse">
            ...
            <tbody>
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="p-3 border"><?php echo e($bill->employee->name ?? 'N/A'); ?></td>
                    <td class="p-3 border"><?php echo e($bill->employee->position->name ?? 'N/A'); ?></td>
                    <td class="p-3 border"><?php echo e($bill->month->month ?? ''); ?> - <?php echo e($bill->month->year ?? ''); ?></td>
                    <td class="p-3 border text-center"><?php echo e(number_format($bill->total_quantity, 2)); ?></td>
                    <td class="p-3 border text-right">रु <?php echo e(number_format($bill->total_amount, 2)); ?></td>
                       ...
                       <?php if(auth()->user()->role === 'admin' || \App\Models\RolePermission::where('role', auth()->user()->role)->where('permission', 'petrol.bills.manage')->exists() || $bill->isEdit): ?>
                        ...
                        <?php if(auth()->user()->role === 'admin' || (auth()->user()->role && \App\Models\RolePermission::where('role', auth()->user()->role)->where('permission', 'petrol.bills.manage')->exists())): ?>
                            <form action="<?php echo e(route('petrol.bills.toggleEdit', $bill->id)); ?>" method="POST" class="inline">
                                <?php echo csrf_field(); ?>
                                <button type="submit" class="<?php echo e($bill->isEdit ? 'bg-orange-500 hover:bg-orange-600' : 'bg-green-600 hover:bg-green-700'); ?> text-white px-3 py-1 rounded text-sm ml-1">
                                    <?php echo e($bill->isEdit ? 'Edit बन्द गर्नुहोस्' : 'Edit खोल्नुहोस्'); ?>
            ...
            <a href="<?php echo e(route('petrol.bills.index', array_merge(request()->all(), ['export' => 'excel']))); ?>" class="bg-green-600 text-white px-3 py-2 rounded text-sm hover:bg-green-700 inline-block mb-3">
    Excel डाउनलोड
</a>
        </table>
    </div>
